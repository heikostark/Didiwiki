...
http_request_add_param ( HttpRequest *req, char *key, char *value )
{
    if ( req->n_params_alloc <= req->n_params ) {
        ...
        req->params = realloc ( req->params, req->n_params_alloc * sizeof ( HttpRequestParam ) );
...
http_request_param_showall ( HttpRequest *req, HttpResponse *res )
{
    ...
    http_response_printf ( res, "<html><body>\n" );
...
http_request_param_get ( HttpRequest *req, char *key )
{
    ...
    for ( i=0; i<req->n_params; i++ ) if ( !strcmp ( req->params[i].key, key ) ) return req->params[i].val;
    return NULL;
}
...
http_request_new ( void )
{
    ...
    if ( fgets ( request_line, sizeof ( request_line ), stdin ) == NULL ) malformed_request();

    if ( ( token = util_extract_token ( request_line, &z ) ) == NULL ) malformed_request();

    if ( ! ( !strcmp ( token,"GET" ) || !strcmp ( token,"POST" ) || !strcmp ( token,"HEAD" ) ) ) malformed_request();

    putenv ( "GATEWAY_INTERFACE=CGI/1.0" );
    putenv ( util_mprintf ( "REQUEST_METHOD=%s",token ) );

    if ( ( token = util_extract_token ( z, &z ) ) == NULL ) malformed_request();

    putenv ( util_mprintf ( "REQUEST_URI=%s", token ) );

    if ( token ) req->uri = strdup ( token );
    ...
    if ( token ) req->path_info = strdup ( token );

    putenv ( util_mprintf ( "QUERY_STRING=%s", &token[i] ) );

    if ( &token[i] ) req->query_string = strdup ( &token[i] );

    if ( getpeername ( fileno ( stdin ), ( struct sockaddr* ) &remoteName, &size ) >=0 ) putenv ( util_mprintf ( "REMOTE_ADDR=%s", inet_ntoa ( remoteName.sin_addr ) ) );
    ...
    */
    ...
    if ( getenv ( "QUERY_STRING" ) ) http_request_parse_params ( req, strdup ( ( getenv ( "QUERY_STRING" ) ) ), '&' );
    ...
    if ( getenv ( "HTTP_COOKIE" ) ) http_request_parse_params ( req, strdup ( ( getenv ( "HTTP_COOKIE" ) ) ), '&' );
...
http_response_append_header ( HttpResponse *res, char *header )
{
    if ( res->extra_headers ) {
        res->extra_headers = realloc ( res->extra_headers,strlen ( res->extra_headers ) + strlen ( header ) + 1 );
...
http_server ( int iPort )
{
    ...
    unsigned int       lenaddr;       /* Length of the inaddr structure */
    ...
    while ( 1 ) {
        if ( nchildren>MAX_PARALLEL ) {
            /* Slow down if connections are arriving too fast */ sleep ( nchildren-MAX_PARALLEL );
        }
        delay.tv_sec = 60;
        delay.tv_usec = 0;
        FD_ZERO ( &readfds );
